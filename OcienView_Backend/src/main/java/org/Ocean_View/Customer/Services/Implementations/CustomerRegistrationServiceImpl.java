...
import org.Ocean_View.Customer.Services.CusUniqueID;
...
public class CustomerRegistrationServiceImpl implements RegistrationService {
    ...
    private final CusUniqueID idGenerator;
    private final HashPassword hashPassword;

    public CustomerRegistrationServiceImpl() {
        this.idGenerator = new CusUniqueID();
        this.hashPassword = new HashPassword();
    }

    public CustomerRegistrationServiceImpl(CusUniqueID idGenerator, HashPassword hashPassword) {
    ...
    public Customer register(RegisterRequest request) {
        ...
        try {
            ...
            String sql = "INSERT INTO customers (unique_id, firstName, lastName, email, password, nic, contactNumber, status, created_at,image_path) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
            ...
            if (affectedRows > 0) {
                ...
                if (rs.next()) {
                    ...
                    String selectSql = "SELECT * FROM customers WHERE unique_id = ?";
    ...
    public boolean isNICExists(String nic) {
        String sql = "SELECT COUNT(*) FROM customers WHERE nic = ?";
